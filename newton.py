...
warnings.simplefilter("error", np.exceptions.ComplexWarning) #to ensure we are not dropping complex perturbations
...
def computeJacobian(modelfun,x, options, bReturnResult):
    ...
    if options['bVectorisedModelFun']:
        ...
        if options['bUseComplexStep']:
            ...
            for ip in range(n_x):
                perturbation = np.zeros(np.size(x), dtype='complex128')
